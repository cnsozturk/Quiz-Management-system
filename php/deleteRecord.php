...
the code in catch-block is executed. */
    ...
    try {
        $sql = "DELETE FROM $dbname.Students WHERE StudentID = :SID";
        $stmnt = $conn->prepare($sql);     // read about prepared statements here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
        $stmnt->bindParam(':SID', $_POST['sid']);   // :stdId is the variable that we used in $sql, there must be a colon (:) in front of it.
