...
try {
	$sql="INSERT INTO Students (StudentID, FName, LName, Mail) VALUES (:SID, :FN, :LN, :ML);";   // all the variable names must start with a colon (:)
	$stmnt = $conn->prepare($sql);    // read about prepared statement here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
	$stmnt->bindParam(':SID', $_POST['sid']);   // stdId in $_POST['stdId'] in the exact name of the input element in HTML. if any typo, your code does not work   
	$stmnt->bindParam(':FN', $_POST['fn']);   // note the single quotes, If you forget to put single quotes, your code does not work.
	$stmnt->bindParam(':LN', $_POST['ln']);
	$stmnt->bindParam(':ML', $_POST['ml']);
